...
class AppTest extends TestCase
{
    ...
    public function testBlog() {
        ...
        $response = $app->run($request);
        $this->assertContains('<h1>Welcome to the Blog</h1>', (string)$response->getBody());
        $this->assertEquals(200, $response->getStatusCode());

        $requestSingle = new ServerRequest('GET', '/blog/test-article');
        $responseSingle = $app->run($requestSingle);
        $this->assertContains('<h1>Welcome to the article : test-article</h1>', (string)$responseSingle->getBody());
    }
    ...
    public function testError404() {
        $app = new App();
        $request = new ServerRequest('GET', '/fakeurl');
        $response = $app->run($request);
